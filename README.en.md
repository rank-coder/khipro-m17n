[বাংলায় পড়ুন](README.md)
## Our Website:
### https://KhiproTeam.github.io/khipro/

# Khipro khipro-m17n

![khipro-m17n](https://socialify.git.ci/rank-coder/khipro-m17n/image?description=1&forks=1&issues=1&language=1&logo=https%3A%2F%2Fraw.githubusercontent.com%2Frank-coder%2Fkhipro-m17n%2Fmain%2Fbn-khipro.png&name=1&pattern=Circuit%20Board&pulls=1&stargazers=1&theme=Auto)

This is the m17n version of Khipro. Using it with Typing-booster provides even better predictions and suggestions.
- Khipro layout is available on **Linux**, **Windows**, and **Android**.

> [!NOTE]
> This project is powered by github 🌟s. Go ahead and *star* it please!

# Documentation

## Introduction
While Avro is popular for Bangla phonetic typing, frequent use of the Shift key and distance of certain keys (e.g., ^ for ঁ) can disrupt typing speed and flow. This makes it difficult to increase speed beyond a certain limit.

To overcome these limitations, we developed the Khipro layout, inspired by China's Pinyin system. Because Pinyin is case-insensitive and features intelligent suggestions, Chinese users can often type in Chinese faster than English.

Khipro brings that same concept—Bangla can be written quickly and smoothly without using Shift. All necessary characters can be typed without moving your hands away from the main keyboard area.

On Linux, Khipro becomes even faster through Typing Booster, providing suggestions for multiple upcoming words. On Windows, Android, and other platforms, predictive text features can be used with apps that implement Khipro.

In Khipro, it's possible to force a vowel sign (*kar*) to become a vowel or vice versa with just a single button press. There are many more such features.


## Demo Video:

[Screencast From 2025-03-19 22-43-44.webm](https://github.com/user-attachments/assets/84f803a1-d01e-416e-addb-65c7f7cd6b91)



## Khipro Layout at a Glance
![Khipro Layout at a Glance](https://khiproteam.github.io/khipro/quickstart/%E0%A6%95%E0%A7%8D%E0%A6%B7%E0%A6%BF%E0%A6%AA%E0%A7%8D%E0%A6%B0-%E0%A6%8F%E0%A6%95-%E0%A6%A8%E0%A6%9C%E0%A6%B0%E0%A7%87.png)

## [Khipro Quickstart](https://KhiproTeam.github.io/khipro/quickstart)

Our website contains instructions, tips, and tricks for using Khipro.  
Visit https://KhiproTeam.github.io/khipro/quickstart

## [Live Demo](https://khiproteam.github.io/khipro/quickstart/#libh-tipn-dm)
Try Khipro without installation: [Click Here](https://khiproteam.github.io/khipro/quickstart/#libh-tipn-dm).



# Contact
1. Khipro Telegram Group: https://t.me/KhiproChat
2. Discord: https://discord.gg/GPt6s8cb48
3. Bangla Localization Community Telegram Group: https://t.me/BanglaLocalizationCommunity
4. Linux Bangla Group: https://t.me/linux_bangla
